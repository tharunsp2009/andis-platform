...
import { LicenseService, SubscriptionService } from "./server_licensing.ts";
...
async function startServer() {
  ...
  app.post("/api/license/downgrade", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `Successfully downgraded to ${tier}`,
      });
    } catch (error) {
  ...
  app.post("/api/devices/enroll", async (req, res) => {
    try {
      ...
      const validation = await LicenseService.validateDeviceLimit(
        orgId,
        currentDeviceCount
      );
  ...
  // Simulated subscription trial expired endpoint
  app.post("/api/license/simulate-expiration", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `License status simulated as ${status}`,
      });
    } catch (error) {
      ...
      const activeConnections = connections
        ...
        .map((c) => {
          const isExternal =
            ...
            !c.peerAddress.startsWith("127.0.");

          const isSuspiciousPort = SUSPICIOUS_PORTS.includes(
            Number(c.peerPort)
          );
  ...
  // API route to get security logs
  ...
  setInterval(async () => {
    try {
      ...
      const realEntries = connections
        ...
        .map((c) => {
          const isSuspicious = TELEMETRY_SUSPICIOUS_PORTS.includes(
            Number(c.peerPort)
          );
      ...
      const isAnomalousTick = Math.random() < 0.2;
      ...
      const randomIp =
        remoteIps[Math.floor(Math.random() * remoteIps.length)];

      const randomPort = isAnomalousTick
        ? [4444, 1337, 5555, 6666][Math.floor(Math.random() * 4)]
        : [80, 443, 22, 5432, 6379, 8080][
            Math.floor(Math.random() * 6)
          ];

      const isSuspicious =
        TELEMETRY_SUSPICIOUS_PORTS.includes(randomPort);
      ...
      const simulatedEntry = {
        ...
        local_ip: "10.0.2.15",
        local_port:
          Math.floor(Math.random() * (49151 - 1024 + 1)) + 1024,
        remote_ip: randomIp,
  ...
  server.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(
      `WebSocket Proxy active: ws://localhost:${PORT} -> ws://localhost:8765`
    );
  });
}
